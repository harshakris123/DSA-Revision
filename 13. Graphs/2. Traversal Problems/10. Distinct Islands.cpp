...
- During the DFS, we keep track of the path taken to traverse each island. The path can be
 represented as a string, where each character in the string represents the direction of movement during 
 the DFS (U for Up, D for Down, R for Right, and L for Left).
- We use a set to store the paths of all distinct islands. As sets store unique elements, 
we will have only unique paths in the set.
