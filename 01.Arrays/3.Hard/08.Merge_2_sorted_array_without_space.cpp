...
1. Initialize three pointers: i, j, and k, where i points to the last valid element of nums1,
 j points to the last element of nums2, and k points to the last index of the merged array nums1.
...
void merge(vector<int>& nums1, int m, vector<int>& nums2, int n) {
    ...
    // We only copy remaining elements from nums2 because if nums1 has leftovers, they are already in the correct sorted position.
    while (j >= 0) {
        swap(nums2[j], nums1[k]);
        j--;
        k--;
